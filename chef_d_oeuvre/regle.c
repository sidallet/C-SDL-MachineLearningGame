...
Regle chargerRegle(FILE* flux) {
	Regle regle;
	fscanf(flux, "[%d %d %d %d %d][%d %d][%d] -> %d (%d)", 
...
void afficherRegle(FILE* flux, const Regle regle)
{
    ...
    fprintf(flux, "[%d] -> %d (%d)\n", regle.colone_double, regle.decis, regle.priorite);
}
...
TabRegle alterTabRegle(TabRegle tabRegle) {
	...
	int random_num = rand() % 15;  // Génère un nombre aléatoire entre 0 et 6 
	...
	if (random_num < 5) { //Modif observation
		...
		tabRegle.regles[iRegle].colone_double = rand_dif(tabRegle.regles[iRegle].colone_double, -2, 2) * 5;
	}
